...
export async function ensureCanonicalSyncSchema() {
    ...
    await pool.query(`
        ALTER TABLE canonical_sync_jobs
...
export async function enqueueCanonicalSyncJob(job, client = pool) {
    ...
    const { rows } = await client.query(
        ...
        INSERT INTO canonical_sync_jobs (
            ...
            last_error,
            updated_at
        ) VALUES ($1, $2, $3, $4::jsonb, 1, $5, 0, now(), NULL, now())
        ON CONFLICT (job_key)
        DO UPDATE SET
            payload = EXCLUDED.payload,
            version = canonical_sync_jobs.version + 1,
            status = EXCLUDED.status,
            attempts = 0,
            next_attempt_at = now(),
            last_error = NULL,
            updated_at = now()
        RETURNING job_key, version
        `,
        [
            jobKey,
            job.resourceType,
...
async function markJobSuccess(jobKey, version) {
    const { rowCount } = await pool.query(
        ...
        WHERE job_key = $1
          AND version = $3
        `,
        [jobKey, JOB_STATUS.SYNCED, version],
    );

    return rowCount > 0;
}

async function markJobFailure(jobKey, version, attempts, errorMessage) {
    ...
    const { rowCount } = await pool.query(
        ...
        WHERE job_key = $1
          AND version = $5
        `,
        [jobKey, JOB_STATUS.FAILED, nextAttemptAt, errorMessage, version],
    );
...
export async function processCanonicalSyncJob(jobKey) {
    ...
    try {
        ...
        const marked = await markJobSuccess(job.job_key, job.version);
        if (!marked) {
            logger.warn(
                { jobKey: job.job_key, version: job.version },
                "canonical sync success ignored for stale job version",
            );
        }
        return { ok: true, processed: true };
    } catch (error) {
        const message = getErrorMessage(error);
        const marked = await markJobFailure(job.job_key, job.version, job.attempts, message);
        if (!marked) {
            logger.warn(
                { jobKey: job.job_key, version: job.version },
                "canonical sync failure ignored for stale job version",
            );
            return { ok: true, processed: true, stale: true };
        }
    ...
    for (const job of rows) {
        try {
            ...
            if (!marked) {
                logger.warn(
                    { jobKey: job.job_key, version: job.version },
                    "canonical sync retry success ignored for stale job version",
                );
            }
        } catch (error) {
            const message = getErrorMessage(error);
            ...
            if (!marked) {
                logger.warn(
                    { jobKey: job.job_key, version: job.version },
                    "canonical sync retry failure ignored for stale job version",
                );
                continue;
            }
